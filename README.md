...
[Download files](http://bit.ly/advr16-1) 
...
1. `typeof(NA)` *NA logical*
2. `typeof(NA_real_)` *NA double*
3. `typeof(NA_integer_)` *NA integer*
4. `typeof(NA_character_)` *NA chr*
    
### Data Structures

List vs. vector vs. matrix vs. data.frame  
> `str(list(TRUE, 1))` *Each element of a list can be different*   
> `c(1, c(2, 3, c(4, 5)))` *Flat vector*  
> `str(list(1, list(2, 3, list(4, 5))))` *Recursive list (nested list)*  
...
`str()` for lists and more compact printing: see options for `?str()`, namely max.level and list.len

### Subsetting

What can you put inside `[]`?

1. Logical; consider `NA` vs. `NA_integer_`
...
4. Zero integer (`0L`) where you return 0. Not really practically useful, sometimes in testing  
...
6. Nothing, which just returns everything. Could be useful for e.g. printing data.tables. Or subsetting n-dim.  
    * Consider: `mtcars[1:5, ]` or `mtcars[, c("vs", "am")]`  
    * Consider: `x[] <- y` vs. `x <- y` (later)  
        

### Memory usage
`x <- rnorm(1e6)`   
`pryr::object.size(x)` *8 MB*  

`x1 <- list(x, x, x)`  
`pryr::object.size(x1)` *Still 8 MB*   
...
In the development version of RStudio 1.0, `Ctrl + Enter` (`Ctrl + R`) will run the entire argument chain, instead of the
...
* This is preferable to e.g. `rm(list=ls())`

Using `rm(list=ls())` does not actually reset the workspace-- does not clear loaded package(s).
...
Formal arguments vs. arguments; consider:  
    `myfun <- f(x) { x + 1}`    
`x` refers to the formal argument (formals)  

`myfun(5)`  
`5` is the argument (of the function `myfun()`)  

Infix functions are the ordinary functions, and these go in-between, e.g. `2 * 3` where `*` is infix  
Prefix functions take Infix and put backticks around, e.g. `*`(2, 3)`    
...
Better: `if(length(x) != length(y)) stop("length of x must equal length of y")`  
...
`tibbles` or `data.table`s are probably a better option, or at least safer/stricter.  
...
Be careful using `which()`; logical algebra almost always clearer  
...
Consider:  
`df[, vars]` 
This is type-unstable because of the drop arg not being explicitly set as FALSE  
...
If `options()` are modified in the current environment, this can also throw things off-- so you must handle this.  
Other things: `na.action()`, time zone, system language, text encoding.  

Only ever use `options()` to control side-effects of a function. The value of an option should never affect the return 
value of a function.  
...
`df[] <- map(df, fix_missing)` *`[]` keeps as data.frame, modifying in-place(?)*
...
Versus:  
`str(map_chr(df[1:2], class))`    
`str(map_chr(df[3:4], class))`  
`map_chr(df[1:4], ~class(.)[1])`    
`map_chr(df[1:4], ~class(.))` # error  

Insert `browser()` into function to debug  

If something takes a data.frame, what happens about input of matrix? What should it return?  
    - One way is to enforce input for non-df, via e.g. `as.data.frame()`  
...
Consider:  
`urls <- c("https://google.com", "https://wikipedia.org")`
...
The goal of generic functions is to let others extend.  `mean()` just calls `UseMethod("mean"` where different 
...
`tibbles` have different methods from data.frames. One specific example is the `print()` method for `tibble`s.  
...
# Ways to epresent the following:  
x <- c(-1, 0, 1, 2, 3)`  
...
Use `methods()` to find methods.  E.g. `methods("mean")`  

In R, methods belong to generic functions.  
`methods(class = "data.frame")`  

Can also use the method name, e.g. `[.factor`.  
    * More reliable is to use `getS3method()`
...
In R, this paradigm (refer to table in notes) is transposed, i.e. functions have methods for objects.  
...
choose from in R. Should probably stick with S3.  
